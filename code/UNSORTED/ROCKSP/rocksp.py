...
רמת קושי: 2
-----------------
המשחק "אבן, מספריים, נייר" הוא משחק קלאסי לשני שחקנים, שבו כל שחקן בוחר אחת משלוש אפשרויות: אבן, מספריים או נייר. המנצח נקבע לפי הכללים הבאים: אבן מנצחת מספריים, מספריים מנצחות נייר, ונייר מנצח אבן. אם שני השחקנים בוחרים באותה אפשרות, מוכרז תיקו.
כללי המשחק:
1.  השחקן והמחשב בוחרים אחת משלוש אפשרויות: 1 - אבן, 2 - מספריים, 3 - נייר.
2.  המחשב מבצע בחירה אקראית.
3.  המנצח נקבע לפי הכללים הבאים:
    * אבן (1) מנצחת מספריים (2).
    * מספריים (2) מנצחות נייר (3).
    * נייר (3) מנצח אבן (1).
4.  אם שניהם בוחרים באותה אפשרות, זהו תיקו.
5.  תוצאת כל סיבוב מוצגת על המסך.
6.  המשחק מסתיים לאחר כל סיבוב.
-----------------
אלגוריתם:
1. בקש מהמשתמש לבחור: 1 - אבן, 2 - מספריים, 3 - נייר.
2. אם קלט המשתמש אינו 1, 2 או 3, הצג הודעת שגיאת קלט וחזור לשלב 1.
3. הגרל בחירה אקראית עבור המחשב: 1, 2 או 3.
4. הצג על המסך את בחירת המשתמש ובחירת המחשב.
5. השווה את בחירת המשתמש ובחירת המחשב:
   5.1 אם שתי הבחירות זהות, הצג "TIE".
   5.2 אם המשתמש בוחר 1 (אבן) והמחשב 2 (מספריים), הצג "YOU WIN".
   5.3 אם המשתמש בוחר 1 (אבן) והמחשב 3 (נייר), הצג "I WIN".
   5.4 אם המשתמש בוחר 2 (מספריים) והמחשב 1 (אבן), הצג "I WIN".
   5.5 אם המשתמש בוחר 2 (מספריים) והמחשב 3 (נייר), הצג "YOU WIN".
   5.6 אם המשתמש בוחר 3 (נייר) והמחשב 1 (אבן), הצג "YOU WIN".
   5.7 אם המשתמש בוחר 3 (נייר) והמחשב 2 (מספריים), הצג "I WIN".
6. סוף המשחק.
-----------------
תרשים זרימה:
```mermaid
flowchart TD
    Start["התחלה"] --> InputUserChoice["<p align='left'>בקשת בחירת המשתמש:
    <code><b>
    ...
    ValidateInput -- כן --> GenerateComputerChoice["<p align='left'>הגרלת בחירה אקראית של המחשב:
    <code><b>
...
מקרא:
    ...
    ErrorMessage - הצגת הודעת שגיאת קלט.
    GenerateComputerChoice - הגרלת בחירה אקראית עבור המחשב: 1, 2 או 3.
    OutputChoices - הצגת בחירת המשתמש ובחירת המחשב על המסך.
...
def play_rock_paper_scissors():
    while True:
        # בקשת בחירת המשתמש
        try:
            user_choice = int(input("הכנס את בחירתך (1 - אבן, 2 - מספריים, 3 - נייר): "))
        except ValueError:
            print("קלט לא תקין. אנא הכנס מספר 1, 2 או 3.")
            continue
        ...
        # הגרלת בחירה אקראית עבור המחשב
        computer_choice = random.randint(1, 3)

        # הצגת בחירת המשתמש והמחשב
        print("בחירתך:", user_choice)
        print("בחירת המחשב:", computer_choice)
...
1. **ייבוא מודול `random`**:
   - `import random`: מייבא את מודול `random`, המשמש להגרלת בחירה אקראית עבור המחשב.

2. **פונקציה `play_rock_paper_scissors()`**:
   - מגדירה פונקציה המכילה את לוגיקת המשחק "אבן, מספריים, נייר".

3. **לולאה `while True:`**:
   - לולאה אינסופית הנמשכת עד לקליטת קלט תקין וקביעת מנצח.

4. **קלט נתונים ובדיקת תקינות**:
    - `try...except ValueError`: מטפל בשגיאות אפשריות במקרה שהמשתמש יכניס ערך שאינו מספר.
   -   `user_choice = int(input("הכנס את בחירתך (1 - אבן, 2 - מספריים, 3 - נייר): "))`: מבקש מהמשתמש את בחירתו וממיר אותה למספר שלם.
    -  `if user_choice not in [1, 2, 3]:`: בודק שהמספר שהוכנס הוא 1, 2 או 3. אם הקלט אינו תקין, מוצגת הודעת שגיאה, והלולאה מתחילה מחדש.

5. **הגרלת בחירת המחשב**:
   - `computer_choice = random.randint(1, 3)`: מגריל מספר שלם אקראי בין 1 ל-3 כולל ושומר אותו במשתנה `computer_choice`.

6.  **הצגת בחירת המשתמש והמחשב**:
     -  `print("בחירתך:", user_choice)`: מציג על המסך את בחירת המשתמש.
     -  `print("בחירת המחשב:", computer_choice)`: מציג על המסך את בחירת המחשב.

7. **קביעת המנצח**:
   -   `if user_choice == computer_choice:`: בודק האם הבחירות זהות. אם כן, מוצגת הודעה על תיקו.
   -  `elif (user_choice == 1 and computer_choice == 2) or \
         (user_choice == 2 and computer_choice == 3) or \
         (user_choice == 3 and computer_choice == 1):`: בודק את כל המקרים שבהם המשתמש מנצח.
   -    `print("ניצחת!")`: מציג הודעה על ניצחון המשתמש.
   -    `else:`: אם אף אחד מהתנאים הקודמים לא התקיים, המחשב ניצח.
        - `print("הפסדת!")`: מציג הודעה על הפסד המשתמש.

8. **סיום המשחק**:
   -   `break`: מסיים את סבב המשחק הנוכחי.
9. **הפעלת המשחק**:
   -  `if __name__ == "__main__":`: בלוק זה מבטיח שהפונקציה `play_rock_paper_scissors()` תופעל רק אם הקובץ מורץ ישירות ולא מיובא כמודול.
   -  `play_rock_paper_scissors()`: קורא לפונקציה כדי להתחיל את המשחק.
"""