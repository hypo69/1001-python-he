...
def start_task_1():
    ...
    log_message("Первая задача запущена.") # המשימה הראשונה הופעלה.

def stop_task_1():
    global scheduler_active_1
    scheduler_active_1 = False
    schedule.clear("task1")
    log_message("Первая задача остановлена.") # המשימה הראשונה נעצרה.

def start_task_2():
    global scheduler_active_2
    scheduler_active_2 = True
    schedule.every(1).seconds.do(do_something_else).tag("task2")
    log_message("Вторая задача запущена.") # המשימה השנייה הופעלה.

def stop_task_2():
    global scheduler_active_2
    scheduler_active_2 = False
    schedule.clear("task2")
    log_message("Вторая задача остановлена.") # המשימה השנייה נעצרה.

def remove_task_1():
    schedule.clear("task1")
    log_message("Первая задача удалена.") # המשימה הראשונה הוסרה.

def remove_task_2():
    schedule.clear("task2")
    log_message("Вторая задача удалена.") # המשימה השנייה הוסרה.

def list_tasks():
    log_message("Запланированные задачи:") # משימות מתוזמנות:
